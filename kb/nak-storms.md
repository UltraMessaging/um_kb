...
Durign this incoming-traffic-based overload, packet loss is inevitable.
The question is: what happens when the incoming traffic burst subsides returns to normal?
...
this is a evidence of a NAK storm.
...
The best tay to prevent NAK storms it to prevent NAKs.
See [[Packet Loss]] for instructions to reduce and possibly eliminate packet loss.
...
1. UM version 6.9 contains fixes a problem with the
...
message rates and configuration. But as a rule of thumb, UM's lost packet recovery
...
For users with larger scales, we recommend sub-dividing your network into
multiple
...
component to pass messages between the TRDs. A common way of sub-dividing a
large network that spans multiple geographically dispersed data centers is to
define each data center as a TRD and using DROs to interconnect them.
Contact UM support for advice on how to implmement this.

## Prevention Vs. Avoidance

While it is possible to fully prevent NAK storms from happening,
the required tradeoffs are are sometimes too costly.
For example, getting packet loss close to zero requires that publishers
be configured to block if they burst their data too intensly.
...
Make sure your applications can handle double or triple the expected message rate.
